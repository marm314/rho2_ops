...
//
  ...
  kind = 6;
  cgqfgh ( order, kind, alpha, beta, a, b, x, w );
//
//  Write the rule.
//
  r = new double[2];
  ...
  rule_writegh ( order, filename, x, w, r );
//
//  Free memory.
//
  delete [] r;
...
//
  ...
  zemu = class_matrixgh( kind, nt, alpha, beta, aj, bj );
//
//  Compute the knots and weights.
//
  sgqfgh ( nt, aj, bj, zemu, t, wts );
...
//
  cdgqfgh ( nt, kind, alpha, beta, t, wts );
//
...
//
  ...
  scqfgh ( nt, t, mlt, wts, nt, ndx, wts, t, kind, alpha, beta, a, b );
...
//
{
  ...
  parchkgh ( kind, 2 * m - 1, alpha, beta );
  ...
  for ( l = 1; l <= n; l++ )
  {
    ...
    for ( ; ; )
    {
      for ( m = l; m <= n; m++ )
      {
        ...
        if ( fabs ( en[m-1] ) <= prec * ( fabs ( d[m-1] ) + fabs ( d[m] ) ) )
        {
          break;
        }
      }
      ...
      g = ( d[l] - p ) / ( 2.0 * en[l-1] );
      r =  sqrt ( g * g + 1.0 );
      g = d[m-1] - p + en[l-1] / ( g + fabs ( r ) * r8_signgh ( g ) );
      s = 1.0;
      c = 1.0;
      p = 0.0;
      mml = m - l;
...
void rule_writegh ( int order, string filename, double x[], double w[],
  double r[] )
...
*/
  r8mat_writegh ( filename_w, 1, order, w );
  r8mat_writegh ( filename_x, 1, order, x );
  r8mat_writegh ( filename_r, 1, 2,     r );

  return;
}
//****************************************************************************80

void scqfgh ( int nt, double t[], int mlt[], double wts[], int nwts, int ndx[],
...
void sgqfgh ( int nt, double aj[], double bj[], double zemu, double t[],
  double wts[] )
